...
fs::path get_path() {
    ...
    fs::path p = fs::current_path().parent_path() / "test";
    return p;
}
...
int main(int argc, const char *argv[])
{
    ...
    if(is_apple){
        ...
        string path_string = dir_path.string(); // Converte o caminho para uma representação de string
...
// FORMER loadImage() 
bool loadImage_old(const std::string& img_name, cv::Mat& img){
    string path = src_folder + img_name;
    img = cv::imread(path,IMREAD_COLOR);
    if (img.empty()) {
        std::cerr << "Erro ao carregar a imagem." << std::endl;
